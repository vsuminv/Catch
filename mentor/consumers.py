...
def message_to_json(message):
    return {
        ...
        'author_id': message.user_id.user_id,
        'content': message.message,
        'timestamp': str((message.created_at + timedelta(hours=9)).strftime("%Y/%m/%d %H:%M")),
        'yourimg': '/media/' + str(message.user_id.img)
    }
...
class ChatConsumer(WebsocketConsumer):
    ...
    def new_message(self, data):
        user_id = data['user']
        room_id = int(self.room_name)
        user_contact = User.objects.filter(user_id=user_id)[0]
        room_contact = Room.objects.filter(id=room_id)[0]
        print('this')
        print(user_contact.img)
        print(room_contact)
